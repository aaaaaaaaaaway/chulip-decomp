...
int func_00119840(unsigned char *packet, Sprite *sprite) {
    Quad color;
    int hw;
    int hh;
    ...
    hw = (int)((float)sprite->w * sprite->sx);
    hh = (int)((float)sprite->h * sprite->sy);
    if (sprite->flags & 1) {
        hw = -hw;
    }
    if (sprite->flags & 2) {
