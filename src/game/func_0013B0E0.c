...
extern void func_00101868(const float *value);

void func_0013B0E0(void) {
    struct Actor001ED3C8 *actor;
    struct Actor001ED3C8 *target;
    unsigned short remaining;

    actor = D_001ED3C8;
    remaining = actor->field_0x14.h;
    actor->field_0x14.h = remaining - 1;
    if ((short)remaining > 0) {
        target = D_001ED3C8;
        D_00205060[0] = (float)target->field_0x16 / 10.0f;
        D_00205060[1] = (float)target->field_0x18 / 10.0f;
        D_00205060[2] = (float)target->field_0x1A / 10.0f;
        func_00101868(D_00205060);
        return;
    }
